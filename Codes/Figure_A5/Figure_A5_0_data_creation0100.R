...
## (Ingroup=Senators, Outgroup=Voters, Outsample-N=100)
...
## 100 Outsamples (ingroup: senators, outgroup: voters)
set.seed(20200725)
d <- pblapply(1:ntimes, function(i) {
  data.frame(
    ipbridge_density(dv,dc,100,polarity1=rep(3,2),polarity2=rep(1,2),model="oc")
  )
})

save(d, file=paste0(projdir, "/Outputs/simulation/senator_sim_400_0100s_cand.rda"))
rm(d)