...
## (Ingroup=Candidates, Outgroup=Voters, Outsample-N=500)
...
d <- pblapply(1:ntimes, function(i) {
  data.frame(
    ipbridge_density(utas2_v_p,utas2_c_p,500,polarity1=rep(utas2_rightcand_v,2),
                     polarity2=rep(utas2_rightcand_c,2))
  )
})

save(d, file=paste0(projdir, "/Outputs/simulation/utas12_sim_400_0500s_cand.rda"))
rm(d)