## Ideal Point Analysis for Senator Data
...
tj$party <- c(dcp,dvp)
tj$party <- factor(tj$party, levels=c(1,2), labels=c("Democrat","Republican"))
tj$cv <- c(rep(1,nrow(dc)), rep(2,nrow(dv)))
tj$cv <- factor(tj$cv, levels=c(1,2), labels=c("Senator","Voter"))
ip_j <- tj %>% dplyr::filter(party == "Democrat" | party == "Republican")

###############################
## senators on voters' space ##
###############################
...
tv <- data.frame(t1[[4]])
colnames(tv) <- c("coord1D","coord2D")
tv$cv <- rep(2, dim(tv)[1])
tv$party <- dvp
tc_pro <- data.frame(t1[[2]])
colnames(tc_pro) <- c("coord1D","coord2D")
tc_pro$cv <- rep(1, dim(tc_pro)[1])
tc_pro$party <- dcp_new
tc_reg <- data.frame(t1[[3]])
colnames(tc_reg) <- c("coord1D","coord2D")
tc_reg$cv <- rep(1, dim(tc_reg)[1])
tc_reg$party <- dcp_new

## first, transformed senators and original voters
ip_t1_pro <- rbind.data.frame(tc_pro,tv)
ip_t1_pro <- ip_t1_pro %>% dplyr::filter(party == "Democrat" | party == "Republican")
...
ip_t1_reg <- ip_t1_reg %>% dplyr::filter(party == "Democrat" | party == "Republican")
...
tc_o <- data.frame(t1[[1]])
colnames(tc_o) <- c("coord1D","coord2D")
tc_o$cv <- rep(1, dim(tc_o)[1])
tc_o$party <- dcp_new
## second, original senators and original voters
ip_t1o <- rbind.data.frame(tc_o,tv)
ip_t1o <- ip_t1o %>% dplyr::filter(party == "Democrat" | party == "Republican")
...
## creating IP data (coordinates and party id) by respondents type
tc <- data.frame(t2[[4]])
colnames(tc) <- c("coord1D","coord2D")
tc$cv <- rep(1, dim(tc)[1])
tc$party <- dcp
tv_pro <- data.frame(t2[[2]])
colnames(tv_pro) <- c("coord1D","coord2D")
tv_pro$cv <- rep(2, dim(tv_pro)[1])
tv_pro$party <- dvp_new
tv_reg <- data.frame(t2[[3]])
colnames(tv_reg) <- c("coord1D","coord2D")
tv_reg$cv <- rep(2, dim(tv_reg)[1])
tv_reg$party <- dvp_new

## first, transformed senators and original voters
ip_t2_pro <- rbind.data.frame(tc,tv_pro)
ip_t2_pro <- ip_t2_pro %>% dplyr::filter(party == "Democrat" | party == "Republican")
ip_t2_pro$cv <- factor(ip_t2_pro$cv,  levels=c(1,2), labels=c("Senator","Voter"))

## second, transformed senators and original voters
ip_t2_reg <- rbind.data.frame(tc,tv_reg)
ip_t2_reg <- ip_t2_reg %>% dplyr::filter(party == "Democrat" | party == "Republican")
...
tv_o <- data.frame(t2[[1]])
colnames(tv_o) <- c("coord1D","coord2D")
tv_o$cv <- rep(2, dim(tv_pro)[1])
tv_o$party <- dvp_new
## original senators and original voters
ip_t2o <- rbind.data.frame(tc,tv_o)
ip_t2o <- ip_t2o %>% dplyr::filter(party == "Democrat" | party == "Republican")
...
tmpdir <- projdir
rm(projdir)
save.image(paste0(tmpdir, "/Outputs/application/senator_ip.rda"))